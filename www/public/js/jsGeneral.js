...
let LenguajeParametros = {
  ...
  emptyTable: `<span style="filter: opacity(0.5);"><br><span><svg xmlns="http://www.w3.org/2000/svg" width="35" height="35" fill="currentColor" class="bi bi-search" viewBox="0 0 16 16">
    <path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0z"/>
    </svg></span><br><br> <b>Sin registros</b></p></span>`,
...
let calendarioNormalInput = {
  ...
  locale: localFlatPick,
};

let calendarioNormalSinFecha = {
  altInput: true,
  altFormat: "l j F, Y",
  dateFormat: "Y-m-d",
  // defaultDate: 'today',
...
function pintarOpcionMenuActiva() {
  ...
  let validaruta = `${misaArray[misaArray.length - 2]}/${
    misaArray[misaArray.length - 1]
  }`;
  let cont = 0;
  misA.forEach((a) => {
    if (cont == 0) {
      ...
      let validaopcionruta = `${miopcionRut[miopcionRut.length - 2]}/${
        miopcionRut[miopcionRut.length - 1]
      }`;
...
function Mensaje2(
  tipo = 0,
  contenido = "Accion a realizar",
  mensajeConfirmacion = "Sí, confirmo",
  subtitulo = ""
) {
  switch (tipo) {
    case 0:
...
function imagenPortadaArea(
  e = "reset",
  preview = "reset",
  imgDef = "general/placeholderimage.png"
) {
...
function revisionDatos(
  datos,
  datosTipoEspecial = "",
  tipoMensaje_TipoEspecial = 2,
  titulo_TipoEspecial = "Hubo un problema",
  subtitulo_TipoEspecial = "Intenta nuevamente, si el problema persiste, comunicate con el área de sistemas.",
  textoBoton_TipoEspecial = "Entendido"
) {
  if (!datos.status) {
    if (datos.tipo == "error_bd") {
      Mensaje2(
        ...
        "Oops",
        "Entendido",
        "Al parecer ocurrió un error en la base de datos. Intentar nuevamente en unos minutos mientras se depura el error. Gracias por su comprensión."
      );
    } else if (datos.tipo == datosTipoEspecial) {
      Mensaje2(
        tipoMensaje_TipoEspecial,
        titulo_TipoEspecial,
        textoBoton_TipoEspecial,
        subtitulo_TipoEspecial
      );
    } else {
      Mensaje2(
        0,
        "ERROR",
        "Ok",
        "Error desconocido. Por favor comunicarse con el área de sistemas para resolverlo. Gracias."
      );
    }
    return false;
  }
